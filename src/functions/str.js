...
module.exports.str = targetString => new Parser((parserState) => {
    ...
    if (source.length < nextIndex) {
        return StateUpdater.updateError(parserState, BaseError.createParseError(`Expecting string '${targetString}', but got end of input.`, index));
    }

    const slice = source.slice(index, nextIndex);
    if (slice !== targetString) {
        return StateUpdater.updateError(parserState, BaseError.createParseError(`Expecting string '${targetString}', got '${slice}...'`, index))
    }
